...
# Count number of Pro Bowl draft picks per pick sequence
pro_bowl_pick_count = df[df['PB'] > 0]
pro_bowl_pick_count = pro_bowl_pick_count.groupby('Pick')['PB'].count()
...
plt.title(f'Pro Bowl Draft % by Pick {season_start} - {season_stop} Seasons')
plt.xlabel('Pick')
plt.show()