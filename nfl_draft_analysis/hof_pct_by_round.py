...
pick_count = df.groupby('Round')['AP1'].count()

# Count number of HOF draft picks per round (HOF defined as having >= 3 First Team All Pro (AP1))
hof_pick_count = df[df['AP1'] >= 3]
hof_pick_count = hof_pick_count.groupby('Round')['AP1'].count()
...
plt.title(f'Hall of Fame Draft % by Round {season_start} - {season_stop} Seasons')
plt.xlabel('Round')
plt.show()